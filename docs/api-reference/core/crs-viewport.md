...
* **`MapController` normalization uses Web Mercator math.** With the default `controller: {normalize: true}`, `MapController.applyConstraints` normalizes view state using Web Mercator world coordinates, which causes the view to snap toward the equator on zoom-out and can clamp zoom-out to `log2(viewportHeight/512)`, making the map appear to vanish. When using a non-Mercator `crs`, set `controller: {normalize: false}` to disable normalization (a proper fix delegating this math to the viewport is future work).
