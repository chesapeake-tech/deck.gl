 ...
 * `{x, y, width, height, anchorX?, anchorY?, mask?}`). `IconMapping`/
...
export interface DeckIconMappingEntry {
  ...
  anchorY?: number;
  mask?: boolean;
  /** deck.gl's own atlas rendering doesn't need this (the atlas image is
 ...
 * through (previously dropped; needed by the icon-size mapping) rather than
 * removed. */
