...
export class MapState extends ViewState<MapState, MapStateProps, MapStateInternal> {
  ...
  applyConstraints(props: Required<MapStateProps>): Required<MapStateProps> {
    ...
    // exactly the plain Web Mercator behavior.
    const viewport = maxBounds ? this._tryMakeViewport(props) : null;

    props.zoom = this._constrainZoom(props.zoom, props, viewport);
    ...
    if (maxBounds) {
      ...
      const [minLng, minLat] = unprojectCommonPoint(viewport, [
        bl[0] + halfWidth,
        bl[1] + halfHeight
      ]);
      const [maxLng, maxLat] = unprojectCommonPoint(viewport, [
        tr[0] - halfWidth,
        tr[1] - halfHeight
      ]);
